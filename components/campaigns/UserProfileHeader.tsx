...
export default function UserProfileHeader({ className = '' }: UserProfileHeaderProps) {
  ...
  const displayName = userProfile?.displayName || firebaseUser?.displayName || user?.email?.split('@')[0] || 'User';
      ...
      <div className="relative" ref={dropdownRef}>
        ...
        >
          ...
          {photoURL ? (
            ...
            <div className="w-10 h-10 rounded-full bg-brand-navy text-white flex items-center justify-center font-semibold text-sm">
        ...
        {isOpen && (
          <div className="absolute right-0 top-full mt-2 w-44 bg-white rounded-lg shadow-lg border border-gray-200 py-1 z-50">
            <button
              onClick={() => { setIsOpen(false); router.push('/brands/settings?tab=profile'); }}
            ...
            <button
              onClick={() => { setIsOpen(false); router.push('/brands/settings'); }}
              className="w-full px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 flex items-center gap-2.5 cursor-pointer"
            >
              <Settings size={16} />
              Settings
