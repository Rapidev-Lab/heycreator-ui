...
export default function BrandLoginPage() {
  ...
  const handleSocialAuth = async (provider: string) => {
    ...
    try {
      if (provider === "google") {
        await signInWithGoogle("brand");
      } else if (provider === "facebook") {
        await signInWithFacebook("brand");
      } else if (provider === "apple") {
        await signInWithApple("brand");
      } else {
      ...
      // Re-fetch profile — on first sign-up, onAuthStateChanged fires before
      // the Firestore documents are created, leaving userProfile null.
      await refreshUserProfile();
      redirectBasedOnRole("brand");
    } catch (error: any) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const userCredential = await signInWithEmail(email, password, 'brand');
  ...
  const handleRequestCode = async (e: React.FormEvent) => {
    ...
    try {
      ...
      sessionStorage.setItem("phoneAuthRole", "brand");
      sessionStorage.setItem("phoneAuthNumber", fullPhoneNumber);
      router.push("/auth/otp?role=brand"); // Redirect to OTP page with role
    } catch (error: any) {
  ...
  const handleForgotPassword = () => {
    router.push("/auth/brand/forgot-password");
  };
      ...
      <div className="w-full max-w-xl px-4 md:px-6 lg:px-8">
        ...
        <AuthCard>
          ...
          <div className="text-center mb-6">
            <h2 className="text-sm  text-brand-navy">Login as a Brand</h2>
          </div>
